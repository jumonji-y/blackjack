...
function seyJudge($my_total_hands, $opp_total_hands){
  ...
  }else if($end_game_flg === true && $my_total_hands < 21 && $my_total_hands > $opp_total_hands) {
    echo "勝ちです！おめでとう！";
  }else if($end_game_flg === true && $my_total_hands < 21 && $my_total_hands < $opp_total_hands) {
    echo "残念！負けです";
  }
...
if (!empty($_POST["submit"])) {
    ...
    if(empty($_POST['reset'])){
      ...
      var_dump($_SESSION['dealer_cards']);
    }
    // もしPOST送信(ヒット)されていたら
    if (!empty($_POST['hit'])) {
...
<?php
 ...
 seyJudge($total_player_hands, $total_dealer_hands);
